...
int main(int argc,char** argv){ (void)argc;(void)argv;
    ...
    GUI_CreateLabel(&MyApp,10,42,"Banco:"); EditBanco=GUI_CreateEdit(&MyApp,60,38,120,25,"MERCADO",NULL);
    GUI_CreateLabel(&MyApp,190,42,"Tabela:"); EditTabela=GUI_CreateEdit(&MyApp,240,38,140,25,"PRODUTO",NULL);
    BtnCarregar=GUI_CreateButton(&MyApp,390,38,110,25,"CARREGAR",OnBtnCarregarClick);
    MemoCampos=GUI_CreateMemo(&MyApp,10,70,620,120); gui_set_prop(MemoCampos,PROP_COLOR,0x000000);
    GUI_CreateLabel(&MyApp, 10,  200, "Campo:"); EditCampo = GUI_CreateEdit(&MyApp, 60,  196, 140, 25, "", NULL);
    GUI_CreateLabel(&MyApp, 210, 200, "Novo:"); EditNovo  = GUI_CreateEdit(&MyApp, 250, 196, 110, 25, "", NULL);
    GUI_CreateLabel(&MyApp, 370, 200, "Tam:"); EditTam   = GUI_CreateEdit(&MyApp, 405, 196, 45,  25, "20", NULL);
    GUI_CreateLabel(&MyApp, 460, 200, "TIPO:"); ComboTipo=GUI_CreateComboBoxEx(&MyApp,485,196,70,24,NULL);
    GUI_ComboBoxEx_AddItem(ComboTipo, "text"); GUI_ComboBoxEx_AddItem(ComboTipo, "int");
    BtnAdd=GUI_CreateButton(&MyApp,10,230,110,25,"ADD CAMPO",OnBtnAddClick);
    BtnRen=GUI_CreateButton(&MyApp,130,230,110,25,"RENOMEAR",OnBtnRenClick);
    BtnResize=GUI_CreateButton(&MyApp,250,230,110,25,"RESIZE",OnBtnResizeClick);
    BtnDrop=GUI_CreateButton(&MyApp,370,230,110,25,"DROP CAMPO",OnBtnDropClick);
    MemoLog=GUI_CreateMemo(&MyApp,10,262,620,288); gui_set_prop(MemoLog,PROP_COLOR,0x000000);
    GUI_Memo_AddStr(MemoLog,"Studio Alter v1.0 - manutencao segura de tabelas.\n");
