...
#define SYS_FATREADAT       31
#define SYS_FATWRITEAT      32
#define SYS_FATMKDIR        33
...
static inline int sys_exec(const char* elf_path) {
    if (!elf_path) return -1;
    return (int)_syscall(SYS_EXEC, (uint64_t)elf_path, 0, 0);
}
...
static inline int sys_fat_read_at(const char* name, void* buf, uint32_t size, uint32_t off) {return (int)_do_syscall(SYS_FATREADAT, (uint64_t)(uintptr_t)name, (uint64_t)(uintptr_t)buf, (uint64_t)size, (uint64_t)off, 0);}

static inline int sys_fat_write_at(const char* name, const void* buf, uint32_t size, uint32_t off) {return (int)_do_syscall(SYS_FATWRITEAT, (uint64_t)(uintptr_t)name, (uint64_t)(uintptr_t)buf, (uint64_t)size,(uint64_t)off, 0);}

static inline int sys_fat_mkdir_ex(const char* name) {return (int)_do_syscall(SYS_FATMKDIR, (uint64_t)(uintptr_t)name, 0, 0, 0, 0);}
