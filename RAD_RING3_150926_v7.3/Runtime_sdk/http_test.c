...
// ============================================================================
// v2.0: HELPERS HTTP (seguir redirect)
// ============================================================================
...
// Parseia "http://host/caminho" extraindo host e path
...
// ============================================================================
// Faz UM HTTP GET, exibe a resposta e extrai status + Location
// Retorna o código HTTP (200, 301...) ou -1 em erro
// ============================================================================
static int http_get_once(const char* host, const char* path, uint32_t dns_server,
                         char* out_location, size_t loc_max) {
    uint32_t target_ip = parse_ip(host);
    if (target_ip == 0) {
        target_ip = dns_resolve(host, dns_server);
    ...
    char request[256];
    size_t len = safe_strcpy(request, "GET ", sizeof(request));
    len = safe_strcat(request, path, sizeof(request), len);
    len = safe_strcat(request, " HTTP/1.1\r\nHost: ", sizeof(request), len);
    len = safe_strcat(request, host, sizeof(request), len);
    len = safe_strcat(request, "\r\nUser-Agent: LBF-OS/1.0\r\nConnection: close\r\n\r\n", sizeof(request), len);
    send(sockfd, request, len, 0);

    // Recebe: exibe no memo + acumula o cabeçalho para parsing
    ...
    while (timeout > 0) {
        int bytes = recv(sockfd, rx_buf, sizeof(rx_buf) - 1, 0);
        if (bytes > 0) {
            rx_buf[bytes] = '\0';
            GUI_Memo_AddStr(MemoResponse, rx_buf);
            if (!headers_done) {
                for (int i = 0; i < bytes && hdr_len < sizeof(hdr_buf) - 1; i++)
                    hdr_buf[hdr_len++] = rx_buf[i];
                hdr_buf[hdr_len] = '\0';
                if (local_strstr(hdr_buf, "\r\n\r\n")) headers_done = true;
            }
            timeout = 100;
        } else {
            sys_sleep(10);
    ...
    // Extrai o código de status ("HTTP/1.1 301 ...")
    int status = -1;
    if (hdr_buf[0]=='H' && hdr_buf[1]=='T' && hdr_buf[2]=='T' && hdr_buf[3]=='P' &&
        hdr_buf[4]=='/' && hdr_buf[5]=='1' && hdr_buf[6]=='.') {
        const char* p = hdr_buf + 7;
        while (*p && *p != ' ') p++;
        while (*p == ' ') p++;
        status = 0;
    ...
    // Extrai o header Location (se existir)
    if (out_location) {
        out_location[0] = '\0';
        const char* loc = local_strstr(hdr_buf, "Location:");
        if (loc) {
            loc += 9;
            while (*loc == ' ') loc++;
            size_t i = 0;
...
// TESTE 6 v2.0: HTTP GET + segue redirect 301/302 (google.com -> www.google.com)
void OnBtnTest6Click(void* sender) {
    ...
    TRY {
        ...
        char host[128];
        char path[128];
        safe_strcpy(host, target_str, sizeof(host));
        safe_strcpy(path, "/", sizeof(path));

        int status = -1;
        for (int r = 0; r < 2; r++) {   // no máx. 1 redirect seguido
            char location[192];

            GUI_Memo_AddStr(MemoResponse, "\n[HTTP] GET ");
            GUI_Memo_AddStr(MemoResponse, path);
            GUI_Memo_AddStr(MemoResponse, " em ");
            GUI_Memo_AddStr(MemoResponse, host);
            GUI_Memo_AddStr(MemoResponse, "...\n");
            Flush_Grafico_Janela();

            status = http_get_once(host, path, g_dns_server, location, sizeof(location));

            if ((status == 301 || status == 302) && location[0]) {
                char new_host[128], new_path[128];
                if (parse_url(location, new_host, sizeof(new_host), new_path, sizeof(new_path))) {
                    GUI_Memo_AddStr(MemoResponse, "\n[HTTP] Redirect detectado! Seguindo para ");
                    GUI_Memo_AddStr(MemoResponse, new_host);
                    GUI_Memo_AddStr(MemoResponse, new_path);
                    GUI_Memo_AddStr(MemoResponse, "\n");
                    Flush_Grafico_Janela();
                    safe_strcpy(host, new_host, sizeof(host));
                    safe_strcpy(path, new_path, sizeof(path));
                    continue;   // faz o 2º GET
                }
            }
            break;   // 200 ou erro: termina
        }

        if (status == 200) {
            GUI_Memo_AddStr(MemoResponse, "\n[OK] HTTP 200 OK recebido! Pagina baixada!\n\n");
        } else {
