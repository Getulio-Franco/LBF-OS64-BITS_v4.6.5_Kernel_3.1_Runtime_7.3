...
void wm_remove_window(TForm* form) {
    if (!form || form == g_desktop) return;
    if (g_modal_form == form) g_modal_form = NULL;
...
void wm_handle_mouse_event(int x, int y, int event) {
    ...
    if (!target && g_desktop) {
     ...
     /* ------------------------------------------------------------------
     * PASSO 3: BLOQUEIO MODAL
     * Com um modal ativo, o mundo inteiro some para o mouse:
     * - clique FORA do retângulo do modal: evento consumido (nada age)
     * - clique DENTRO: só o modal existe como alvo
     * ------------------------------------------------------------------ */
    if (g_modal_form) {
        int mx = g_modal_form->Win.Control.Left;
        int my = g_modal_form->Win.Control.Top;
        int mw = g_modal_form->Win.Control.Width;
        int mh = g_modal_form->Win.Control.Height;
        bool inside = (x >= mx && x <= mx + mw && y >= my && y <= my + mh);

        if (!inside) {
            // Solta qualquer arrasto pendente e consome o evento
        ...
        target = g_modal_form;   // clique dentro: só o modal recebe
    }
       ...
       case EVENT_MOUSE_DOWN: { 
            ...
            if (target->BorderStyle == bsNone) {
                ...
                if (y >= target->Win.Control.Top && y <= target->Win.Control.Top + title_height) {
                    ...
                    if (local_y >= 6 && local_y <= 22 && local_x >= (win_w - 22) && local_x <= (win_w - 6)) {
                        if (target != g_modal_form) {          // <<< PASSO 3: NOVO
        ...
        case EVENT_MOUSE_MOVE:
            if (dragging && g_active && g_active != g_desktop && g_active->Win.Draggable) {
                ...
                // <<< HOTFIX: o form modal nunca é reposicionado para 0,0
                if (g_active && !g_active->Win.Draggable && g_active != g_desktop &&
                    g_active != g_modal_form) {
...
void gui_render_form(TForm* form) {
    ...
    // PASS 1: desenha o form e todos os controles filhos na ordem normal
    ...
    // PASS 2 (OVERLAY): popups de ComboBoxEx por cima de TUDO
    for (int i = 0; i < form->ControlCount; i++) {
        TControl* c = form->Controls[i];
        if (!c || !c->Visible) continue;
        if (c->Type == TYPE_COMBOBOX_EX) {
            gui_draw_combobox_ex_popup((TComboBoxEx*)c);
        }
    }
}
...
void wm_handle_keyboard_event(char key) {
    // PASSO 3: com modal ativo, o teclado só fala com ele
    if (g_modal_form) {
        gui_process_key(g_modal_form, key);
        return;
    }
