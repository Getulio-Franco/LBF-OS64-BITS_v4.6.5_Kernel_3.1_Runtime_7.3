...
void sha256_update(sha256_ctx* c, const uint8_t* data, size_t len) {
    ...
    while (len > 0) {
        ...
        if (take > (uint32_t)len) take = (uint32_t)len;
...
void sha256_final(sha256_ctx* c, uint8_t out[32]) {
    ...
    uint8_t b80 = 0x80, z = 0;
    sha256_update(c, &b80, 1);
