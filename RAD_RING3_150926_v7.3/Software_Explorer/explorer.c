...
void Coletar_Slots_Inativos(void) {
    ...
    if (!algum_app_ativo && lblStatus) {
        if (strcmp(lblStatus->Graphic.Control.Caption, "LBF-OS RUNTIME v7.3 [KERNEL 3.0 BANDO DE DADOS]") != 0) {
            strcpy(lblStatus->Graphic.Control.Caption, "LBF-OS RUNTIME v7.3 [KERNEL 3.0 BANDO DE DADOS]");
        }
    }
...
void Setup_Interface_Nativa(void) {
    ...
    lblStatus = gui_create_label((TWinControl*)frmTaskbar, "lblStatus", "LBF-OS RUNTIME v7.3 [KERNEL 3.0 BANDO DE DADOS]");
    if (lblStatus) {
