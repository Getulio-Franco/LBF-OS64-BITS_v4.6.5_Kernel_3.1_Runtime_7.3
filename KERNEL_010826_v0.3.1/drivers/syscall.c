    ...
    switch (syscall_num) {
       ...
       /* case SYS_FATREAD: {
        ...
        case SYS_FATREAD: {
            ...
            // --- FRACIONAMENTO (CHUNKING DE 32 KB) ---
            #define AHCI_CHUNK_SIZE 32768 // 32 KB por ciclo de DMA
            uint32_t total_lido = 0;

            while (total_lido < size) {
                uint32_t a_ler = size - total_lido;
                if (a_ler > AHCI_CHUNK_SIZE) {
                    a_ler = AHCI_CHUNK_SIZE;
                }

                // Lê usando offset incremental para dar folga ao controlador AHCI
                int status = fat32_read_file_at_offset(filename, data + total_lido, a_ler, total_lido);
                
                if (status != FS_SUCCESS) {
                    if (total_lido > 0) return (int)total_lido; // Retorna o que foi lido com sucesso
                    return -1; // Falha se não leu nenhum byte
                }
       ...
       // Chama a função interna do driver FAT32 - fat32_read_file numero: 29 
        ...
        case SYS_CHDIR: {
            ...
            /* 1) aliases da raiz — aceita tudo quanto é jeito de dizer "raiz" */
            if (path[0] == 0 ||
                strcmp(path, "/") == 0 || strcmp(path, "\\") == 0 ||
                strcmp(path, "0:") == 0 || strcmp(path, "0:/") == 0 ||
                strcmp(path, "0:\\") == 0 || strcmp(path, ".") == 0) {
                current_dir_cluster = 2;
                return 0;
            }

            /* 2) tira drive "0:" e barras iniciais */
            const char* p = path;
            if (p[0] == '0' && p[1] == ':') p += 2;
            while (*p == '/' || *p == '\\') p++;
            if (*p == 0) { current_dir_cluster = 2; return 0; }

            /* 3) caminha componente a componente a partir do cwd */
            uint32_t cur = current_dir_cluster & 0x0FFFFFFF;
            if (cur < 2) cur = 2;

            while (*p) {
                /* extrai um componente (ate a proxima barra) */
                ...
                /* ".." = volta pro pai lendo a entrada '..' do cluster atual */
                if (strcmp(comp, "..") == 0) {
                    if (cur == 2) continue;               /* ja esta na raiz */
                    __attribute__((aligned(16))) uint8_t sect[512];
                    if (storage_read_sectors(fat32_current_dev_id,
                        fat32_cluster_to_lba(cur), 1, sect) == 0) return (uint64_t)-3;
                    /* entrada '..' fica na posicao 32 (segunda de 32 bytes) */
                    fat32_directory_entry_t* dd = (fat32_directory_entry_t*)&sect[32];
                    uint32_t par = (((uint32_t)dd->cluster_high << 16) | dd->cluster_low) & 0x0FFFFFFF;
                    cur = (par < 2) ? 2 : par;
                    continue;
                }

                /* componente normal: busca no cluster 'cur' */
                char fat_name[11];
                fat32_to_83_filename(comp, fat_name);   /* maiusculas + padding */
                uint32_t found_c = 0; int achou = 0;
                uint32_t scan = cur;
                while (scan >= 2 && scan < 0x0FFFFFF8 && !achou) {
                    uint32_t lba = fat32_cluster_to_lba(scan);
                    for (uint32_t s = 0; s < disk_bpb.sectors_per_cluster && !achou; s++) {
                        __attribute__((aligned(16))) uint8_t dsect[512];
                        if (storage_read_sectors(fat32_current_dev_id, lba + s, 1, dsect) == 0) return (uint64_t)-3;
                        for (int e = 0; e < 512; e += 32) {
                            if (dsect[e] == 0x00) { scan = 0x0FFFFFFF; break; }  /* fim do dir */
                            if ((uint8_t)dsect[e] == 0xE5) continue;             /* deletado */
                            fat32_directory_entry_t* de = (fat32_directory_entry_t*)&dsect[e];
                            if (memcmp(de->filename, fat_name, 11) == 0) {
                                if (!(de->attributes & 0x10)) return (uint64_t)-2; /* nao eh dir */
                                found_c = (((uint32_t)de->cluster_high << 16) | de->cluster_low) & 0x0FFFFFFF;
                                achou = 1; break;
                            }
                        }
                    }
                    if (!achou) scan = fat32_get_next_cluster(scan) & 0x0FFFFFFF;
                }
                if (!achou) return (uint64_t)-1;
                cur = (found_c < 2) ? 2 : found_c;
            }
            current_dir_cluster = cur;
            return 0;
        }
...
case SYS_NET_RECV: {
    ...
    return ret;
        ...
        //-----------------
case  SYS_FATREADAT: { // SYS_FATREADAT 31— leitura com offset
    const char* name = (const char*)arg1;
    uint8_t* buf     = (uint8_t*)arg2;
    uint32_t size    = (uint32_t)arg3;
    uint32_t off     = (uint32_t)arg4;
    if (!name || !buf || (uint64_t)buf >= 0x8000000000000000ULL) return (uint64_t)-1;
    return (uint64_t)(uint32_t)fat32_read_file_at_offset(name, buf, size, off);
}
case SYS_FATWRITEAT: { // SYS_FATWRITEAT 32— escrita NO OFFSET (random write nativa)
    const char* name = (const char*)arg1;
    uint8_t* buf     = (uint8_t*)arg2;
    uint32_t size    = (uint32_t)arg3;
    uint32_t off     = (uint32_t)arg4;
    if (!name || !buf || size == 0 || (uint64_t)buf >= 0x8000000000000000ULL) return (uint64_t)-1;
    return (uint64_t)(uint32_t)fat32_write_file_at_offset(name, buf, size, off);
}
case SYS_FATMKDIR: { // SYS_FATMKDIR 33— criar subdiretório (para as pastas de banco)
    const char* name = (const char*)arg1;
    if (!name) return (uint64_t)-1;
    return (uint64_t)(uint32_t)fat32_create_dir(name);
}
        //=================
