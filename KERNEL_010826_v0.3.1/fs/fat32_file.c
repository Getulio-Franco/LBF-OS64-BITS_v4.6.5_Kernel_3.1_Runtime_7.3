...
/**
 * @brief Libera uma cadeia de clusters (rollback p/ nao deixar clusters orfaos).
 *        Chamar COM fat32_lock segurado.
 */
static void fat32_free_chain(uint32_t first_c) {
    uint32_t c = first_c & 0x0FFFFFFF;
...
/**
 * @brief Grava/cria um arquivo (overwrite seguro, diretório extensível, sem vazamentos).
 *        Suporta "subdir/arquivo.ext" (1 nivel). size==0 cria arquivo vazio.
 */
int fat32_write_file(const char* name, uint8_t* input_buffer, uint32_t size) {
    int status = FS_SUCCESS;

    K_TRY {
        if (!name || !input_buffer) return FS_ERROR_WRITE;
        spin_lock(&fat32_lock);

        /* 1) Resolve o diretório de destino */
        uint32_t dir_c = current_dir_cluster & 0x0FFFFFFF;
        const char* file_name_part = name;
        int last_slash = -1;
        for (int i = 0; name[i] != '\0'; i++) if (name[i] == '/') last_slash = i;
        if (last_slash != -1 && last_slash < 127) {
            char dir_name[128];
            memcpy(dir_name, name, (size_t)last_slash);
        ...
        /* 2) Overwrite: busca LOCAL na cadeia do diretório-resolvido (sem depender
              de parse de caminho), libera os clusters velhos e marca 0xE5 — tudo
              sob a MESMA trava, sem janela de corrida */
        {
            uint32_t scan_c = dir_c;
            while (scan_c >= 2 && scan_c < FAT32_EOC) {
                uint32_t lba = fat32_cluster_to_lba(scan_c);
                int stop = 0;
                for (uint32_t s = 0; s < disk_bpb.sectors_per_cluster && !stop; s++) {
                    __attribute__((aligned(16))) uint8_t dsect[512];
                    storage_read_sectors(fat32_current_dev_id, lba + s, 1, dsect);
                    for (int i = 0; i < 512; i += 32) {
                        if (dsect[i] == 0x00) { stop = 1; break; }       /* fim do dir */
                        if ((uint8_t)dsect[i] == 0xE5) continue;
                        fat32_directory_entry_t* e = (fat32_directory_entry_t*)&dsect[i];
                        if (memcmp(e->filename, fat_name, 11) == 0) {
                            uint32_t old_c = (((uint32_t)e->cluster_high << 16) | e->cluster_low) & 0x0FFFFFFF;
                            e->filename[0] = (char)0xE5;
                            storage_write_sectors(fat32_current_dev_id, lba + s, 1, dsect);
                            fat32_free_chain(old_c);                     /* sem orfaos */
                            stop = 1;
                            break;
                        }
                    }
                ...
                if (stop) break;
                scan_c = fat32_get_next_cluster(scan_c) & 0x0FFFFFFF;
            }
        }

        /* 3) Aloca a cadeia de dados COM rollback em qualquer falha */
        uint32_t bytes_per_cluster = disk_bpb.sectors_per_cluster * 512;
        uint32_t bytes_left = size;
        uint32_t first_c = 0, prev_c = 0;
        uint8_t* data_ptr = input_buffer;

        while (bytes_left > 0) {
            uint32_t cur = fat32_find_free_cluster() & 0x0FFFFFFF;
            if (cur == 0) {
                if (first_c) fat32_free_chain(first_c);
                spin_unlock(&fat32_lock);
                return FS_DISK_FULL;
            }
            if (first_c == 0) first_c = cur;
            if (prev_c != 0) fat32_set_cluster_entry(prev_c, cur);

            uint8_t* cluster_buf = (uint8_t*)kmalloc(bytes_per_cluster);
            if (!cluster_buf) {
                fat32_free_chain(first_c);
                spin_unlock(&fat32_lock);
            ...
            if (storage_write_sectors(fat32_current_dev_id, fat32_cluster_to_lba(cur),
                                      disk_bpb.sectors_per_cluster, cluster_buf) == 0) {
                kfree(cluster_buf);
                fat32_free_chain(first_c);
                spin_unlock(&fat32_lock);
                return FS_ERROR_WRITE;
            ...
            kfree(cluster_buf);
            fat32_set_cluster_entry(cur, FAT32_EOC);
            data_ptr  += to_write;
            bytes_left -= to_write;
            prev_c = cur;
        }

        /* 4) Procura slot livre no diretório */
        __attribute__((aligned(16))) uint8_t sect[512];
        uint32_t last_dir_c = 0;
        uint32_t scan_c = dir_c;
        while (scan_c >= 2 && scan_c < FAT32_EOC) {
            last_dir_c = scan_c;
            uint32_t lba = fat32_cluster_to_lba(scan_c);
            for (uint32_t s = 0; s < disk_bpb.sectors_per_cluster; s++) {
                ...
                for (int i = 0; i < 512; i += 32) {
                    if (sect[i] == 0x00 || (uint8_t)sect[i] == 0xE5) {
                        ...
                        e->attributes   = 0x20;
                        e->cluster_high = (uint16_t)((first_c >> 16) & 0xFFFF);
                        e->cluster_low  = (uint16_t)(first_c & 0xFFFF);
                        e->file_size    = size;
        ...
        /* 4b) DIRETORIO CHEIO -> estende a cadeia (FAT32 permite dir crescer) */
        if (last_dir_c >= 2 && last_dir_c < FAT32_EOC) {
            uint32_t new_c = fat32_find_free_cluster() & 0x0FFFFFFF;
            if (new_c != 0) {
                uint8_t* zbuf = (uint8_t*)kmalloc(bytes_per_cluster);
                if (zbuf) {
                    memset(zbuf, 0, bytes_per_cluster);
                    uint32_t nlba = fat32_cluster_to_lba(new_c);
                    int wok = 1;
                    for (uint32_t s = 0; s < disk_bpb.sectors_per_cluster; s++)
                        if (storage_write_sectors(fat32_current_dev_id, nlba + s, 1, zbuf) == 0) { wok = 0; break; }
                    kfree(zbuf);
                    if (wok) {
                        fat32_set_cluster_entry(new_c, FAT32_EOC);
                        fat32_set_cluster_entry(last_dir_c, new_c);
                        memset(sect, 0, 512);                 /* cluster novo = dir vazio */
                        fat32_directory_entry_t* e = (fat32_directory_entry_t*)&sect[0];
                        memcpy(e->filename, fat_name, 11);
                        e->attributes   = 0x20;
                        e->cluster_high = (uint16_t)((first_c >> 16) & 0xFFFF);
                        e->cluster_low  = (uint16_t)(first_c & 0xFFFF);
                        e->file_size    = size;
                        storage_write_sectors(fat32_current_dev_id, nlba, 1, sect);
                        spin_unlock(&fat32_lock);
                        return FS_SUCCESS;
                    }
                }
        ...
        /* 5) Sem espaco em lugar nenhum: rollback da cadeia de dados */
        if (first_c) fat32_free_chain(first_c);
        spin_unlock(&fat32_lock);
        status = FS_DISK_FULL;
    }
...
/**
 * @brief Deleta um arquivo e limpa a tabela FAT (com suporte a subpastas).
 */
int fat32_delete_file(const char* filename) {
    ...
    K_TRY {
        ...
        /* Resolve subdiretório e extrai o nome simples do arquivo */
        uint32_t dir_c = current_dir_cluster & 0x0FFFFFFF;
        const char* file_name_part = filename;
        int last_slash = -1;
        for (int i = 0; filename[i] != '\0'; i++) if (filename[i] == '/') last_slash = i;
        if (last_slash != -1 && last_slash < 127) {
            char dir_name[128];
            memcpy(dir_name, filename, (size_t)last_slash);
            dir_name[last_slash] = '\0';
            file_name_part = filename + last_slash + 1;
            fat32_directory_entry_t dir_entry;
            if (fat32_find_entry(dir_name, &dir_entry) != 0 || !(dir_entry.attributes & 0x10)) {
                spin_unlock(&fat32_lock);
                return FS_NOT_FOUND;
...
/**
 * @brief Adiciona dados ao final de um arquivo existente (com suporte a subpastas).
 */
int fat32_append_file(const char* filename, uint8_t* data, uint32_t data_len) {
    ...
    K_TRY {
        ...
        /* 1) Resolve o diretório de destino e extrai o nome do arquivo */
        uint32_t dir_c = current_dir_cluster & 0x0FFFFFFF;
        const char* file_name_part = filename;
        int last_slash = -1;
        for (int i = 0; filename[i] != '\0'; i++) if (filename[i] == '/') last_slash = i;
...
/**
 * @brief Zera um cluster inteiro (usado ao expandir a cadeia de um arquivo).
 */
static void fat32_zero_cluster(uint32_t c) {
    uint32_t bpc = disk_bpb.sectors_per_cluster * 512;
    uint8_t* z = (uint8_t*)kmalloc(bpc);
    if (!z) return;
    memset(z, 0, bpc);
    storage_write_sectors(fat32_current_dev_id, fat32_cluster_to_lba(c), disk_bpb.sectors_per_cluster, z);
    kfree(z);
}

/**
 * @brief Grava dados em uma posição específica (offset) de um arquivo,
 *        SEM apagar o resto do conteúdo (escrita aleatória read-modify-write).
 *        Expande a cadeia de clusters automaticamente se offset+size
 *        ultrapassar o fim atual do arquivo.
 * @return FS_SUCCESS ou código de erro (FS_NOT_FOUND, FS_DISK_FULL, ...)
 */
int fat32_write_file_at_offset(const char* name, uint8_t* buffer, uint32_t size, uint32_t offset) {
    int status = FS_SUCCESS;
    K_TRY {
        if (!name || !buffer || size == 0) return FS_ERROR_WRITE;
        spin_lock(&fat32_lock);

        /* 1) Localiza a entrada de diretório (setor + deslocamento dentro do setor) */
        fat32_directory_entry_t entry;
        uint32_t entry_lba = 0, entry_off = 0;
        if (fat32_find_entry_ext(name, &entry, &entry_lba, &entry_off) != 0) {
            spin_unlock(&fat32_lock);
            return FS_NOT_FOUND;
        }

        uint32_t bpc = disk_bpb.sectors_per_cluster * 512;
        uint32_t first_c = (((uint32_t)entry.cluster_high << 16) | entry.cluster_low) & 0x0FFFFFFF;
        if (first_c < 2 || first_c >= FAT32_EOC) first_c = 0;   /* arquivo vazio */

        /* 2) Caminha (e CONSTROI) a cadeia até o cluster do offset */
        uint32_t target_idx = offset / bpc;
        uint32_t cur = first_c;
        for (uint32_t i = 0; i <= target_idx; i++) {
            if (i == 0) {
                if (cur == 0) {                        /* arquivo ainda sem clusters */
                    cur = fat32_find_free_cluster() & 0x0FFFFFFF;
                    if (!cur) { spin_unlock(&fat32_lock); return FS_DISK_FULL; }
                    fat32_zero_cluster(cur);
                    fat32_set_cluster_entry(cur, FAT32_EOC);
                    first_c = cur;
                }
            } else {
                uint32_t next = fat32_get_next_cluster(cur) & 0x0FFFFFFF;
                if (next < 2 || next >= FAT32_EOC) {   /* precisa expandir a cadeia */
                    next = fat32_find_free_cluster() & 0x0FFFFFFF;
                    if (!next) { spin_unlock(&fat32_lock); return FS_DISK_FULL; }
                    fat32_zero_cluster(next);
                    fat32_set_cluster_entry(cur, next);
                    fat32_set_cluster_entry(next, FAT32_EOC);
                }
                cur = next;
            }
        }

        /* 3) Gravação read-modify-write, cluster a cluster (com checagens) */
        uint32_t pos = offset, remaining = size;
        uint8_t* data = buffer;
        while (remaining > 0) {
            uint32_t internal = pos % bpc;
            uint32_t chunk = (remaining < bpc - internal) ? remaining : (bpc - internal);
            uint8_t* cbuf = (uint8_t*)kmalloc(bpc);
            if (!cbuf) { spin_unlock(&fat32_lock); return FS_ERROR_WRITE; }

            if (storage_read_sectors(fat32_current_dev_id, fat32_cluster_to_lba(cur),
                                     disk_bpb.sectors_per_cluster, cbuf) == 0) {
                kfree(cbuf);
                spin_unlock(&fat32_lock);
                return FS_ERROR_READ;
            }
            memcpy(cbuf + internal, data, chunk);      /* modifica só o trecho pedido */
            if (storage_write_sectors(fat32_current_dev_id, fat32_cluster_to_lba(cur),
                                      disk_bpb.sectors_per_cluster, cbuf) == 0) {
                kfree(cbuf);
                spin_unlock(&fat32_lock);
            ...
            data += chunk; remaining -= chunk; pos += chunk;

            if (remaining > 0) {                       /* avança / expande a cadeia */
                uint32_t next = fat32_get_next_cluster(cur) & 0x0FFFFFFF;
        ...
        /* 4) Atualiza tamanho + 1º cluster na entrada de diretório */
        __attribute__((aligned(16))) uint8_t sect[512];
        if (storage_read_sectors(fat32_current_dev_id, entry_lba, 1, sect) == 1) {
            fat32_directory_entry_t* e = (fat32_directory_entry_t*)(sect + entry_off);
            if (offset + size > e->file_size) e->file_size = offset + size;
        ...
        vga_print_string("[FAT32 Fault] Excecao em fat32_write_file_at_offset!", 0, 39);
        spin_unlock(&fat32_lock);
        status = FS_ERROR_WRITE;
    } K_END_TRY
    return status;
